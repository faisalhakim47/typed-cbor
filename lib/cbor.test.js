...
import { array, bytes, boolean, constant, createEncoder, decode, float, integer, map, nil, oneOf, tag, tagCheck, text, typeCheck, transform, undef } from './cbor.js';
...
describe('Edge Cases', function edgeCaseTests() {
  ...
  describe('Composite Types - transform()', function transformTests() {
    it('should apply encode transformation before encoding', function testTransformEncode() {
      const encoder = createEncoder();
      ...
      const decoded = decode(schema, encoded);
      deepEqual(decoded, 5n);
    });

    it('should apply decode transformation after decoding', function testTransformDecode() {
      const encoder = createEncoder();
      const schema = transform(
        text(),
    ...
    it('should type-check original value before transformation', function testTransformTypeCheck() {
      const encoder = createEncoder();
      const schema = transform(
        integer(),
        (value) => value,
    ...
    it('should work with transform in array', function testTransformInArray() {
      const encoder = createEncoder();
      const schema = array(transform(
        integer(),
        (value) => value,
        (value) => value,
      ));
    ...
    it('should work with transform in map', function testTransformInMap() {
      const encoder = createEncoder();
      const schema = map({
        value: transform(
          integer(),
    ...
    it('should handle transform with nested structure', function testTransformNested() {
      const encoder = createEncoder();
      const schema = transform(
        map({ x: integer(), y: integer() }),
        (obj) => ({ x: Number(obj.x), y: Number(obj.y) }),
  ...
  describe('Composite Types - tag() + transform()', function tagTransformTests() {
    it('should encode and decode a tagged transform with integer', function testTagTransformInteger() {
      const encoder = createEncoder();
      const schema = tag(0n, transform(
        integer(),
        (value) => value,
        (value) => value,
      ));
    ...
    it('should encode and decode a tagged transform with text', function testTagTransformText() {
      const encoder = createEncoder();
      const schema = tag(0n, transform(
        text(),
        (value) => value,
    ...
    it('should encode and decode a tagged transform with complex structure', function testTagTransformComplex() {
      const encoder = createEncoder();
      const schema = tag(0n, transform(
        map({ name: text(), age: integer() }),
        (value) => value,
        (value) => value,
      ));
    ...
    it('should apply encode transformation before encoding', function testTagTransformEncodeTransform() {
      const encoder = createEncoder();
      const schema = tag(0n, transform(
        integer(),
        (value) => value,
    ...
    it('should apply decode transformation after decoding', function testTagTransformDecodeTransform() {
      const encoder = createEncoder();
      const schema = tag(0n, transform(
        text(),
        (value) => value.toLowerCase(),
    ...
    it('should reject wrong tag number during decoding', function testTagTransformMismatch() {
      const encoder = createEncoder();
      const schema = tag(42n, transform(
        integer(),
        (value) => value,
        (value) => value,
      ));
      ...
      const wrongSchema = tag(99n, transform(
        integer(),
        (value) => value,
        (value) => value,
      ));
    ...
    it('should type-check tagged value before encoding', function testTagTransformTypeCheck() {
      const encoder = createEncoder();
      const schema = tag(0n, transform(
        integer(),
        (value) => value,
    ...
    it('should encode and decode tag with transform and nested array', function testTagTransformNestedArray() {
      const encoder = createEncoder();
      const schema = tag(0n, transform(
        array(integer()),
        (value) => value,
    ...
    it('should encode and decode tag with transform and nested map', function testTagTransformNestedMap() {
      const encoder = createEncoder();
      const schema = tag(0n, transform(
        map({ a: integer(), b: text() }),
        (value) => value,
        (value) => value,
      ));
    ...
    it('should work with tag + transform in array', function testTagTransformInArray() {
      const encoder = createEncoder();
      const schema = array(tag(0n, transform(
        integer(),
        (value) => value,
        (value) => value,
      )));
    ...
    it('should work with tag + transform in map', function testTagTransformInMap() {
      const encoder = createEncoder();
      const schema = map({
        taggedValue: tag(0n, transform(
          text(),
          (value) => value,
          (value) => value,
        )),
    ...
    it('should reject non-matching value type during typeCheck', function testTagTransformTypeCheckComplex() {
      const encoder = createEncoder();
      const schema = tag(0n, transform(
        map({ a: integer() }),
        (value) => value,
    ...
    it('should handle tag with transform and bytes value', function testTagTransformWithBytes() {
      const encoder = createEncoder();
      const schema = tag(0n, transform(
        bytes(),
        (value) => value,
    ...
    it('should handle tag with transform and boolean value', function testTagTransformWithBoolean() {
      const encoder = createEncoder();
      const schema = tag(0n, transform(
        boolean(),
        (value) => value,
    ...
    it('should handle tag with transform and nil value', function testTagTransformWithNil() {
      const encoder = createEncoder();
      const schema = tag(0n, transform(
        nil(),
        (value) => value,
    ...
    it('should handle tag with transform and float value', function testTagTransformWithFloat() {
      const encoder = createEncoder();
      const schema = tag(0n, transform(
        float(),
        (value) => value,
    ...
    it('should handle different tag numbers with transform', function testDifferentTagNumbersWithTransform() {
      const encoder = createEncoder();

      const tagNumbers = [0n, 1n, 24n, 100n, 255n, 1000n];

      for (const tagNum of tagNumbers) {
        const schema = tag(tagNum, transform(
          text(),
          (value) => value,
          (value) => value,
        ));
    ...
    it('should handle Date transformation with tag 0', function testTagTransformDateTime() {
      const encoder = createEncoder();
      ...
      const dateTimeCbor = encoder.encode(DateTimeShape, new Date('2024-01-15T10:30:00Z'));
      const dateTime = decode(DateTimeShape, dateTimeCbor);
      equal(dateTime instanceof Date, true, 'shall be Date');
    });

    it('should handle epoch timestamp transformation with tag 1', function testTagTransformTimestamp() {
      const encoder = createEncoder();
      ...
      const timestampCbor = encoder.encode(TimestampShape, new Date('2024-01-15T10:30:00Z'));
      const timestamp = decode(TimestampShape, timestampCbor);
      equal(timestamp instanceof Date, true, 'shall be Date');
    });

    it('should handle custom class transformation', function testTagTransformCustomClass() {
      class Coordinate {
        /**
         * @param {number} x
         * @param {number} y
      ...
      const encoder = createEncoder();
      const CoordinateShape = tag(100n, transform(
        map({ x: integer(), y: integer() }),
        (obj) => new Coordinate(Number(obj.x), Number(obj.y)),
        (coord) => ({ x: BigInt(coord.x), y: BigInt(coord.y) }),
      ));

      const input = new Coordinate(100, 200);
      const cbor = encoder.encode(CoordinateShape, input);
      const output = decode(CoordinateShape, cbor);
      equal(output instanceof Coordinate, true, 'shall be Coordinate');
      deepEqual(output.x, input.x);
      deepEqual(output.y, input.y);
    });
  });
  ...
  describe('tagCheck', function tagCheckTests() {
    ...
    it('should return true when tag with transform matches', function testTagCheckWithTransform() {
      const shape = tag(0n, transform(
        text(),
        (value) => value,
        (value) => value,
      ));
    ...
    it('should return false when tag with transform does not match', function testTagCheckMismatchWithTransform() {
      const shape = tag(42n, transform(
        text(),
        (value) => value,
        (value) => value,
      ));
      ...
      const wrongShape = tag(99n, transform(
        text(),
        (value) => value,
        (value) => value,
      ));
